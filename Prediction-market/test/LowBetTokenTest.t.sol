...
import "../src/BetTokens/LowBetToken.sol";

contract LowBetTokenTest is Test {
    LowBetToken token;
    ...
    function setUp() public {
        vm.prank(owner);
        token = new LowBetToken(owner);
    }

    function test_Constructor_Basic() public {
        assertEq(token.name(), "LowBet Token");
        assertEq(token.symbol(), "LOWBET");
    ...
    function test_SetMinter_Fail_ZeroAddress() public {
        vm.prank(owner);
        vm.expectRevert(
            bytes("LowBetToken: Minter cannot be the zero address")
        );
    ...
    function test_Mint_Fail_NotMinter() public {
        ...
        vm.expectRevert("LowBetToken: Caller is not the authorized minter");
        vm.prank(nonOwner);
        token.mint(user, mintAmount);
    }
    ...
    function test_Mint_Fail_MaxSupplyExceeded() public {
        ...
        vm.prank(testminter);
        vm.expectRevert("LowBetToken: Minting exceeds maximum supply");
        token.mint(user, 1);
    }
}
